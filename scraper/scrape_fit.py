...
def parse_division(code, meta):
    ...
    for el in body.find_all(["h2", "h3", "h4", "h5", "table"]):
        ...
        for tr in el.find_all("tr"):
            ...
            # Round cell only has real text on the first row of a group
            # (rowspan) — subsequent rows in the same round still have the
            # cell present but empty. Track column count via offset, but
            # only overwrite current_round when the cell actually has text,
            # so blank continuation cells don't wipe out the remembered round.
            offset = 0
